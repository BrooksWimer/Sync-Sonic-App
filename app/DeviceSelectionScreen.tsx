  ...
  const refreshBoth = async () => {
    if (connectedDevice) {
      try {
        ...
        // And refresh paired devices
        await fetchPairedDevicesFromPi();
      } catch (error) {
        console.error('Error refreshing devices:', error);
        Alert.alert('Error', 'Failed to refresh device lists');
      }
    }
  };
  ...
  return (
    <YStack style={{ flex: 1, backgroundColor: bg }}>
      ...
      <View style={{ padding: 10, alignItems: 'center' }}>
        ...
        <Button 
          onPress={refreshBoth}
          style={{ marginTop: 10, backgroundColor: pc }}>
          <Text style={{ color: 'white' }}>Refresh All Devices</Text>
        </Button>
      </View>
      ...
      <Button onPress={handleCreate} style={[styles.button, { backgroundColor: pc }]}>
        <Text style={{ color: 'white' }}>Create Configuration</Text>
      </Button>
    </YStack>
  );
}
